...
function restoreLocalSession() {
  ...
  currentView = session.view === "profile" ? "profile" : "community";
}
...
function getDashboardProgress(participant) {
  if (getDashboardCompletedGoalNotice(participant)) {
...
function getDashboardCompletedGoalNotice(participant) {
  const notice = getActiveCompletedGoalNotice(participant);
  if (!notice || hasActiveGoal(participant)) return null;
  return notice;
}
...
function renderResults() {
  ...
  const championParticipants = rankedParticipants.filter((participant) =>
    Boolean(getDashboardCompletedGoalNotice(participant)),
  );
  const activeParticipants = rankedParticipants
    .filter((participant) => !getDashboardCompletedGoalNotice(participant) && hasLeaderboardActivity(participant))
    .slice(0, 3);
